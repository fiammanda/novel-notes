...
const sites = [
  {
    ...
    async parse({ url, update }) {
      ...
      if (url.startsWith("https://changdunovel.com/t/")) {
        ...
        bid = new URL(resp.url).searchParams.get("book_id");
      ...
      const id = createHash("md5").update(url).digest("hex").slice(0, 10);
      if (!update && await redis.exist(id)) {
        throw new Error("URL already exsists");
      }
      ...
      if (item.status === "连载" && new Date(item.update) < Date.now() - 1728000000) {
        item.status = "停更";
      }
      return item;
    }
  ...
  for (const url of urls) {
    try {
      ...
      if (!site) {
        ...
        errs.push({ url, msg: "URL not supported" });
      }
